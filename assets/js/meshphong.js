...
const envMapHash = {
  none: 0,
  reflectivity: new THREE.TextureLoader().load(CubeReflectionMapping),
  refractionRatio: new THREE.TextureLoader().load(CubeRefractionMapping),
};
...
let envMapDropdown = materialFolder.add(guiOptions, "Env Map", Object.keys(envMapHash)).onChange((value) => {
  boxMat.envMap = envMapHash[value];
  boxMat.needsUpdate = true;
  console.log("updated", value);
});

materialFolder.add(boxMat, "reflectivity", 0, 1).name("Reflectivity").listen();
materialFolder.add(boxMat, "refractionRatio", 0, 1).name("Refraction Ratio").listen();
